...
def fetch(spec: SourceSpec, client: httpx.Client) -> SourceFetchResult:
    ...
    for record in records:
        ...
        application_url = record.get("absolute_url")
        ...
        posted_at = parse_iso_date(
            record.get("first_published") or record.get("created_at"),
        )
        candidates.append(Candidate(
            company=spec.parameters.get("employer", spec.name),
            title=title.strip(),
            location=location_name if isinstance(location_name, str) else "",
            ...
            description_text=clean_description(record.get("content")),
